...
function UserActivitiesTable() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        // <Link to={`/mc/${CONSTANTS.ROUTES['overview']}}`}>
        <div className='   gap-2 rounded-lg'>
          <p className='text-[0.71rem]  '>{row.getValue('activity_type')}</p>
        </div>
        // </Link>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        // <Link to={`/mc/${CONSTANTS.ROUTES['overview']}}`}>
        <div className='   gap-2 rounded-lg  '>
          <p className=' text-[0.71rem] '>{row.getValue('activity')}</p>
        </div>
        // </Link>
      ),
    },
  ...
  const { isLoading, data } = useQuery({
    queryKey: [`get-user-activites`],
    queryFn: () => contentService.getUserActivities(),
