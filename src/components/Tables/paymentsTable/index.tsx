...
import { cn, checkStatus, formatDate, formatNaira } from 'lib/utils';
...
function PaymentsTable() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        // <Link to={`/mc/${CONSTANTS.ROUTES['overview']}}`}>
        <div className='  gap-2 rounded-lg'>
          <p className=' text-[0.71rem]  '>{formatNaira(Number(row.getValue('amount') ?? 0))}</p>
        </div>
        // </Link>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        ...
        <p className={`text-[0.71rem] ${checkStatus(row.getValue('status'))}`}>
          {row.getValue('status')}
        </p>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        // <Link to={`/mc/${CONSTANTS.ROUTES['overview']}}`}>
        <div className='   gap-2 rounded-lg  '>
          <p className=' text-[0.71rem] '>{row.getValue('reference')}</p>
        </div>
        // </Link>
      ),
    },
  ...
  const { isLoading, data } = useQuery({
    queryKey: [`get-user-payments`],
    queryFn: () => contentService.getPayments(),
